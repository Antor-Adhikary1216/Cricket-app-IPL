...
const playersPromise = playersdata()
 ...
 <img className="mx-auto mt-4" src={fream} alt="" / >
    ...
    <div className="max-w-330 mx-auto mt-10 flex justify-between">
     ...
     <div>
    <button onClick={()=> seToggle(true)}  className={`btn btn-active mr-1 ${toggle=== true? "bg-amber-400":""}`}>Available</button>
    <button onClick={()=> seToggle(false)} className={`btn btn-active ${toggle===true?"":"bg-amber-300"}`}>Selected <span>(0)</span></button>
     </div>
    ...
    </Suspense>: <Selectedplayes></Selectedplayes>
    }
